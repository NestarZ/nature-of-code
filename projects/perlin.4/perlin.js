...
class Tree {
  ...
  display () {
    if (frameCount % 20 == 0){
      ...
      //text('|(' + (this.id +"")+ ')|', this.xpos, this.ypos);
  ...
  split () {
    if (this.depth < 10 && this.child_n < this.max_child) {
      for (let index = 0; index < int(random(1, 3)); index++) {
        trees[trees_id] = new Tree(trees_id, this.xpos, this.ypos, map(noise(time), 0, 1, 20, 65+90), this.depth+1);
        trees_id++;
        this.child_n++;
      }
    }
