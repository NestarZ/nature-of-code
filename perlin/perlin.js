  ...
  split () {
    if (this.depth < 10 && this.child_n < this.max_child) {
      for (let index = 0; index < int(random(1, 4)); index++) {
        trees[trees_id] = new Tree(trees_id, this.xpos, this.ypos, map(noise(time), 0, 1, 45, 90+45), this.depth+1);
        trees_id++;
        this.child_n++;
      }
    }
    else {
      fill(color(10,70 * noise(this.id),20, this.depth*2));
      stroke(color(5,25 * noise(this.id),10, this.depth*3));
